...
- **이벤트 적재량**: SIM_SPEED 10배속 기준 설비 3대가 분당 수십 건을 만든다.
  oee-service에 시간 단위 롤업 + 보존 정책(기본 7일)이 내장되어 매시 자동 실행된다.
  보존기간은 `EVENT_RETENTION_DAYS`로 조정 (Railway 무료 티어면 2~3일 권장).
